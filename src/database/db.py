...
def teacher_login(username, password):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, username, password, name FROM teachers WHERE username = %s",
        (username,),
    )
    row = cursor.fetchone()
    conn.close()
...
def get_all_students():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT student_id, name, face_embedding, voice_embedding FROM students"
    )
...
def create_student(new_name, face_embedding=None, voice_embedding=None):
    ...
    cursor.execute(
        "INSERT INTO students (name, face_embedding, voice_embedding) VALUES (%s, %s, %s)",
...
def create_subject(subject_code, name, section, teacher_id):
    ...
    try:
        cursor.execute(
            "INSERT INTO subjects (subject_code, name, section, teacher_id) VALUES (%s, %s, %s, %s)",
...
def get_teacher_subjects(teacher_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT subject_id, subject_code, name, section, teacher_id FROM subjects WHERE teacher_id = %s",
        (teacher_id,),
    )
    rows = cursor.fetchall()
    ...
    for r in rows:
        ...
        # Count students
        cursor.execute(
            "SELECT COUNT(*) FROM subject_students WHERE subject_id = %s", (subject_id,)
        )
        sub["total_students"] = cursor.fetchone()[0]

        # Count unique sessions (classes)
        cursor.execute(
            "SELECT COUNT(DISTINCT timestamp) FROM attendance_logs WHERE subject_id = %s",
            (subject_id,),
        )
...
def enroll_student_to_subject(student_id, subject_id):
    ...
    try:
        cursor.execute(
            "INSERT INTO subject_students (student_id, subject_id) VALUES (%s, %s)",
...
def unenroll_student_to_subject(student_id, subject_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "DELETE FROM subject_students WHERE student_id = %s AND subject_id = %s",
        (student_id, subject_id),
    )
    conn.commit()
    conn.close()
...
def get_subject_by_code(subject_code):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT subject_id, name, subject_code FROM subjects WHERE subject_code = %s",
        (subject_code,),
    )
    row = cursor.fetchone()
    conn.close()
...
def check_enrollment(student_id, subject_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM subject_students WHERE student_id = %s AND subject_id = %s",
        (student_id, subject_id),
    )
    row = cursor.fetchone()
    conn.close()
